...
def display_briefing() -> None:
    ...
    **Important:** Across all samples you will be presented, we assume, that there are no confound features in the images, meaning that the model is not tricked by any artifacts or other features that are not related to the **ground truth** object. So this is nothing you need to worry about, but rather a note for You to understand the task better.
