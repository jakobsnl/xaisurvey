...
# Show examples before starting evaluation
    ...
    That's where **explanation methods** come into play. These methods highlight image regions that were most important for the model's decision. In this survey, your task is to assess how well these highlighted regions match the actual features of the object in the image.

    **Goal**: We want to understand how well explanations align with the **ground truth** (i.e., the real object in the image). Your responses provide a human perspective that goes beyond what automated metrics can capture.
    ...
    **Relevance of Explanation**: Does the importance map not related to the **ground truth** appear meaningful for classifying the image?
    ...
    As a consequence, the perfect importance map looks by nature different for each methods. 
    ...
    Make youself familiar with different methods and their resulting explanation shape/ style by looking at the examples below. Afterwards, proceed to the survey."""
