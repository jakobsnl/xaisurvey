    ...
    def find(self, query):
        ...
        for d in self.docs:
            ...
            if "sample" in query and d["sample"] != query["sample"]:
                continue
            # reserved filter
            if d["reserved_until"] and d["reserved_until"] >= query["$or"][1]["reserved_until"]["$lt"]:
                continue
            result.append(d)
        return result
...
def draw_samples(session_state):
    ...
    for folder in selected_folders:
        ...
        while result is None:
            ...
            min_methods_seen = min(session_state.method_count[m["method"]] for m in available)
            candidates = [m for m in available if session_state.method_count[m["method"]] == min_methods_seen]
