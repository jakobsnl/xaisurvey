...
def visibility_info() -> None:
    ...
    """
    st.info(
        "Consider 'wide mode' in the settings or 'cmd +' / 'ctrl +' to zoom manually for better visibility."
    )
...
def display_dummy() -> str | None:
    ...
    alignment = st.radio(alignment_map['question'],
                         alignment_map['scale'],
                         index=None,
...
def display_check(drawn_sample: dict) -> str | None:
    ...
    if drawn_sample.get('type') == 'attention':
        answer = st.radio(drawn_sample['question'],
                          drawn_sample['scale'],
                          index=None,
...
def evaluate_check(drawn_sample) -> None:
    ...
    increase_font_size()
    st.markdown(
        f'### Sample {st.session_state.current_index + 1} of {len(st.session_state.samples)}'
    )
    ...
    explanation_path = EXAMPLE_IMAGES[list(EXAMPLE_IMAGES.keys())[
        st.session_state.current_index %
        len(EXAMPLE_IMAGES)]][st.session_state.current_index % 2][0]
...
def get_relevance() -> str | None:
    ...
    relevance = st.radio(relevance_map['question'],
                         relevance_map['scale'],
                         index=None,
...
def evaluate_explanation(drawn_sample: dict) -> None:
    ...
    explanation_path = os.path.join(IMAGE_FOLDER, object_folder, method,
                                    threshold)
